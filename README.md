...
> [!IMPORTANT]
> This is an unofficial community wrapper and an early-stage project. It depends on the rapidly evolving `@deepseek-ai/dsh@0.1.0-rc.7`. The macOS builds are not Apple-notarized, and the Windows builds are not commercially code-signed.
...
The project currently pins `@deepseek-ai/dsh@0.1.0-rc.7` for reproducible packaging. [`config/dsh-upstream.json`](config/dsh-upstream.json) records the upstream release tag and commit that npm version was built from, and `npm run dsh:check` reports whether a newer DeepSeek Harness release is available.
