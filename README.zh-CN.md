...
> [!IMPORTANT]
> 本项目是非官方社区封装，目前仍属于早期版本，并依赖快速演进中的 `@deepseek-ai/dsh@0.1.7-rc.2`。上游另有官方 Electron 桌面端（上游 monorepo 的 `apps/desktop`）；本项目保持为基于已发布 `@deepseek-ai/dsh` npm 包的独立社区封装，有意不引入该代码——见[决策记录](docs/plans/desktop-port-decision.md)。macOS 构建尚未经过 Apple 公证，Windows 构建尚未进行商业代码签名。
...
当前固定使用 `@deepseek-ai/dsh@0.1.7-rc.2`，以保证打包结果可复现。[`config/dsh-upstream.json`](config/dsh-upstream.json) 记录该 npm 版本对应的上游 release tag 与 commit，`npm run dsh:check` 可检查官方是否发布了更新版本。该版本位于 npm 的 `next` 通道，`latest` 仍指向 `0.1.5-rc.3`。
